...
- 这个线段树一般用于维护区间取 $\max$ ，区间取 $\min$ ，维护历史最大值等操作
- 就比如对于这个例题，[【模板】线段树 3](https://www.luogu.com.cn/problem/P6242)
- 在普通线段树中，区间操作对于区间内的数是均等的，但是例如区间取 $\min$ 这个操作就不均等，因为只有大于 $val$ 的数要被修改，那我们这时候修改就有点难了，所以我们考虑什么时候修改什么时候不修改，而这道题的瓶颈主要在于区间加后如何维护历史最大和区间取 $\min$ 
- 对于前者，我们要想当你一直对一个区间区间加的时候，懒标记还未下传，那么什么时候有可能使历史最大值更新呢，那就是 $lazy$ 值最大的时候，所以我们要维护一个历史 $lazy$ 最大值，如何下传即可
- 对于后者，我们考虑只在区间仅有一种数大于 $val$ 时进行修改，所以我们维护一个次大值，仅在 $se<val<max$ 的时候修改这个区间，那么此时就是将这个大的数减去 $max-val$ ，那么就是区间减了，所以前者的操作我们也要同时维护两个值，那么就有四个 $lazy$ ：*区间最大值懒标记，区间非最大值懒标记，区间最大值懒标记的最大值，区间非最大值懒标记的最大值* 
- 在懒标记下传时，判断一个最大值是否在自己的区间，如何下传对应的懒标记
- 如果有其他的操作，我们一般也要存两个值，既当前值和历史最大值
```cpp
struct ST{
    #define ls x*2
    #define rs x*2+1
...
    ll sum[SN],maxa[SN],maxb[SN],cnt[SN],se[SN]; //区间和，A最大值，B最大值，最大值数量，次大值
    ll add1[SN],add2[SN],add3[SN],add4[SN];//区间最大值懒标记，区间非最大值懒标记，区间最大值懒标记的最大值，区间非最大值懒标记的最大值

    void push_up(int x,int l,int r){
        sum[x]=sum[ls]+sum[rs];
        maxa[x]=max(maxa[ls],maxa[rs]);
        maxb[x]=max(maxb[ls],maxb[rs]);
        if(maxa[ls]==maxa[rs]){
            se[x]=max(se[ls],se[rs]);
            cnt[x]=cnt[ls]+cnt[rs];
        }
        else if(maxa[ls]>maxa[rs]){
            se[x]=max(se[ls],maxa[rs]);
            cnt[x]=cnt[ls];
        }
        else {
            se[x]=max(maxa[ls],se[rs]);
            cnt[x]=cnt[rs];
        }
    }
    void change(int x,ll k1,ll k2,ll k3,ll k4,int l,int r){
        sum[x]+=(k1*cnt[x])+k2*(r-l+1-cnt[x]);
        cmax(maxb[x],maxa[x]+k3);
        maxa[x]+=k1;
        if(se[x]!=-2e9) se[x]+=k2;
        cmax(add3[x],add1[x]+k3);
        cmax(add4[x],add2[x]+k4);
        add1[x]+=k1;
        add2[x]+=k2;
    }
...
        if(maxn==maxa[ls]) change(ls,add1[x],add2[x],add3[x],add4[x],l,mid);
        else change(ls,add2[x],add2[x],add4[x],add4[x],l,mid);
        if(maxn==maxa[rs]) change(rs,add1[x],add2[x],add3[x],add4[x],mid+1,r);
        else change(rs,add2[x],add2[x],add4[x],add4[x],mid+1,r);
        add1[x]=add2[x]=add3[x]=add4[x]=0;
    }

    void build(int x,int l,int r){
        if(l==r){
            sum[x]=maxa[x]=maxb[x]=a[l];
            cnt[x]=1;
            se[x]=-2e9;
            return ;
        }
...
    void modify_add(int x,int l,int r,int ql,int qr,ll val){
        if(ql<=l&&r<=qr){
            sum[x]+=1ll*(r-l+1)*val;
            maxa[x]+=val;
            cmax(maxb[x],maxa[x]);
            if(se[x]!=-2e9) se[x]+=val;
            add1[x]+=val;
            add2[x]+=val;
            cmax(add3[x],add1[x]);
            cmax(add4[x],add2[x]);
...
        }
        push_down(x,l,r);
        if(ql<=mid) modify_add(ls,l,mid,ql,qr,val);
        if(qr>mid) modify_add(rs,mid+1,r,ql,qr,val);
        push_up(x,l,r);
    }
    
    void modify_min(int x,int l,int r,int ql,int qr,ll val){
        if(val>=maxa[x]) return ;
        if(ql<=l&&r<=qr&&se[x]<val){
            ll k=maxa[x]-val;
            sum[x]-=k*cnt[x];
            maxa[x]-=k;
            add1[x]-=k;
...
        }
        push_down(x,l,r);
        if(ql<=mid) modify_min(ls,l,mid,ql,qr,val);
        if(qr>mid) modify_min(rs,mid+1,r,ql,qr,val);
        push_up(x,l,r);
    }
    
...
        if(qr>mid) ans+=query_sum(rs,mid+1,r,ql,qr);
        return ans;
    }
    
    ll query_maxa(int x,int l,int r,int ql,int qr){
        if(ql<=l&&r<=qr) return maxa[x];
        ll ans=-2e9;
        push_down(x,l,r);
        if(ql<=mid) cmax(ans,query_maxa(ls,l,mid,ql,qr));
        if(qr>mid) cmax(ans,query_maxa(rs,mid+1,r,ql,qr));
        return ans;
    }
    
    ll query_maxb(int x,int l,int r,int ql,int qr){
        if(ql<=l&&r<=qr) return maxb[x];
        ll ans=-2e9;
        push_down(x,l,r);
        if(ql<=mid) cmax(ans,query_maxb(ls,l,mid,ql,qr));
        if(qr>mid) cmax(ans,query_maxb(rs,mid+1,r,ql,qr));
        return ans;
    }
}Seg;
```
...
- 我们设计矩阵，首先数据矩阵为
$$\begin{vmatrix}a&b&0\end{vmatrix}$$
- 其中 $a$ 为当前最大值，$b$ 为历史最大值，我们设计加矩阵和赋值矩阵，运算是 $Min-Plus$ 运算
$$Add=\begin{vmatrix}k&k&-\inf\\-\inf&0&-\inf\\-\inf&-\inf&0\end{vmatrix}\quad Set=\begin{vmatrix}-\inf&-\inf&-\inf\\-\inf&0&-\inf\\k&k&0\end{vmatrix}$$
- 类似普通线段树区间加区间赋值即可
