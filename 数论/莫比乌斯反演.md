...
$$g(x)=\sum_{d\mid x}d\cdot\varphi(d)$$
- 让我们来推导一下：
...
## $\sum\limits_{i=1}^{n}\sum\limits_{j=1}^{m} \mathbf{lcm}(i,j)$ 

$$\sum\limits_{i=1}^{n}\sum\limits_{j=1}^{m} \frac{i\cdot j}{\gcd(i,j)}$$
- 枚举公因数，那么剩下的 $\gcd$ 得互质，默认 $n\le m$
$$\sum\limits_{d=1}^{n}d\sum\limits_{i=1}^{\frac{n}{d}}\sum\limits_{j=1}^{\frac{m}{d}} i\cdot j\cdot[\gcd(i,j)]$$
- 我们考虑单独计算后边的东西，设：
$$sum(n,m)=\sum\limits_{i=1}^{n}\sum\limits_{j=1}^{m} i\cdot j\cdot[\gcd(i,j)]$$
- 上边计算过，可以知道
$$sum(n,m)=\sum\limits_{d=1}^{n}\mu(d)\cdot d^{2}\sum\limits_{i=1}^{\frac{n}{d}}\sum\limits_{j=1}^{\frac{m}{d}}i\cdot j$$
- 我们再设：
$$g(n,m)=\sum\limits_{i=1}^{n}\sum\limits_{j=1}^{m} i\cdot j=\frac{n(n+1)}{2}\cdot \frac{m(m+1)}{2}$$
- 那么就有：
$$sum(n,m)=\sum\limits_{d=1}^{n}\mu(d)\cdot d^{2}\cdot g\left( \frac{n}{d}, \frac{m}{d} \right)$$
$$\sum\limits_{d=1}^{n}d\cdot sum\left( \frac{n}{d}, \frac{m}{d} \right)$$
- 对它们依次整除分块即可
